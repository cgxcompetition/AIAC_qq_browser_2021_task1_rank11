...
class FeatureParser:
    ...
    def parse(self, features):
        ...
        query_asr_input_ids, query_asr_mask = self._parse_title(features['query_asr_text'])
        candidate_asr_input_ids, candidate_asr_mask = self._parse_title(features['candidate_asr_text'])
        ...
        return {'input_query_ids': input_query_ids, 'query_mask': query_mask,
                ...
                'query_asr_input_ids': query_asr_input_ids, 'query_asr_mask': query_asr_mask,
                'candidate_asr_input_ids': candidate_asr_input_ids, 'candidate_asr_mask': candidate_asr_mask,
                ...
                'query_labels': query_labels, 'candidate_labels': candidate_labels,
                'query_vid': features['query_vid'], 'candidate_vid': features['candidate_vid'],
                'score': features['score']}

    def parse_val_dataset(self, features):
        input_ids, mask = self._parse_title(features['title'])
        asr_input_ids, asr_mask = self._parse_title(features['asr_text'])
        ...
        return {'input_ids': input_ids, 'mask': mask,
                'asr_input_ids': asr_input_ids, 'asr_mask': asr_mask,
    ...
    def create_dataset(self, files, training, batch_size):
        ...
        if training:
            dataset = dataset.shuffle(buffer_size=batch_size * 16, reshuffle_each_iteration=True)
