...
class FeatureParser:
    ...
    def _encode(self, title):
        ...
        encoded_inputs = self.tokenizer(
            title, max_length=self.max_bert_length, padding='max_length', truncation=True)
        input_ids = encoded_inputs['input_ids']
        mask = encoded_inputs['attention_mask']
        return input_ids, mask

    def _parse_title(self, title):
        input_ids, mask = tf.py_function(
            self._encode, [title], [tf.int32, tf.int32])
    ...
    def _sample(self, frames):
        ...
        if average_duration == 0:
            ...
            offsets = np.multiply(
                list(range(self.max_frames)), average_duration)+average_duration//2
            return [frames[i] for i in offsets], num_frames

    def _parse_frames(self, frames):
        frames = tf.sparse.to_dense(frames)
        frames, num_frames = tf.py_function(
            self._sample, [frames], [tf.string, tf.int32]
        )        
        frames_embedding = tf.map_fn(lambda x: tf.io.decode_raw(
            x, out_type=tf.float16), frames, dtype=tf.float16)
        frames_embedding = tf.cast(frames_embedding, tf.float32)
        frames_embedding.set_shape(
            [self.max_frames, self.args.frame_embedding_size])        
    ...
    def parse(self, features):
        input_query_ids, query_mask = self._parse_title(
            features['query_title']
        )
        input_candidate_ids, candidate_mask = self._parse_title(
            features['candidate_title']
        )
        query_frames, query_num_frames = self._parse_frames(
            features['query_frame_feature']
        )
        candidate_frames, candidate_num_frames = self._parse_frames(
            features['candidate_frame_feature']
        )
        query_labels = self._parse_labels(features['query_tag_id'])
        candidate_labels = self._parse_labels(features['candidate_tag_id'])
        return {
            ...
            'query_num_frames': query_num_frames,
            'candidate_frames': candidate_frames,
            'candidate_num_frames': candidate_num_frames,
            'query_vid': features['query_vid'],
            'candidate_vid': features['candidate_vid'],
    ...
    def create_dataset(self, files, training, batch_size, train_idx):
        @tf.function
        def filter_function(i, data):
            return tf.py_function(lambda x: x in train_idx, inp=[i], Tout=tf.bool)
        if training:
        ...
        dataset = dataset.map(lambda x: tf.io.parse_single_example(
            x, feature_map), num_parallel_calls=AUTOTUNE)
    ...
    def create_val_dataset(self, files, training, batch_size, val_vid_set):
        @tf.function
        def filter_function(data):
            return tf.py_function(lambda x: x.numpy().decode() in val_vid_set, inp=[data['vid']], Tout=tf.bool)
        if training:
            np.random.shuffle(files)
        dataset = tf.data.TFRecordDataset(files, num_parallel_reads=AUTOTUNE)
        ...
        dataset = dataset.map(self.parse_val_dataset,
...
def create_datasets(args, train_idx, val_vid_set):
    train_files = glob.glob(args.train_record_pattern)
    val_files = glob.glob(args.val_record_pattern)

    parser = FeatureParser(args)
    train_dataset = parser.create_dataset(
        train_files, training=True, batch_size=args.batch_size, train_idx=train_idx)
    val_dataset = parser.create_val_dataset(
        val_files, training=False, batch_size=args.val_batch_size, val_vid_set=val_vid_set)
...
def create_pretrain_datasets(args):
    ...
    parser = FeatureParser(args)
    train_dataset = parser.create_val_dataset(
        train_files, training=True, batch_size=args.batch_size)
    val_dataset = parser.create_val_dataset(
        val_files, training=False, batch_size=args.val_batch_size)
