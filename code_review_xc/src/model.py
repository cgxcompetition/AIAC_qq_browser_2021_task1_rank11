...
class CGXMultiModal(Model):
    def __init__(self, config, *args, **kwargs):
        ...
        self.asr_bert = TFBertModel.from_pretrained(config.bert_dir)
    ...
    def call(self, inputs, **kwargs):
        query_bert_embedding = self.bert([inputs['input_query_ids'], inputs['query_mask']])[1]
        query_asr_embedding = self.asr_bert([inputs['query_asr_input_ids'], inputs['query_asr_mask']])[1]
        query_frame_num = tf.reshape(inputs['query_num_frames'], [-1])
        query_vision_embedding = self.nextvlad([inputs['query_frames'], query_frame_num])
        query_vision_embedding = query_vision_embedding * tf.cast(tf.expand_dims(query_frame_num, -1) > 0, tf.float32)
        query_final_embedding = self.fusion([query_vision_embedding, query_bert_embedding, query_asr_embedding])

        candidate_bert_embedding = self.bert([inputs['input_candidate_ids'], inputs['candidate_mask']])[1]
        candidate_asr_embedding = self.asr_bert([inputs['candidate_asr_input_ids'], inputs['candidate_asr_mask']])[1]
        candidate_frame_num = tf.reshape(inputs['candidate_num_frames'], [-1])
        candidate_vision_embedding = self.nextvlad([inputs['candidate_frames'], candidate_frame_num])
        candidate_vision_embedding = candidate_vision_embedding * tf.cast(tf.expand_dims(candidate_frame_num, -1) > 0,
                                                                          tf.float32)
        candidate_final_embedding = self.fusion(
            [candidate_vision_embedding, candidate_bert_embedding, candidate_asr_embedding])
    ...
    def get_variables(self):
        if not self.all_variables:  # is None, not initialized
            self.bert_variables = self.bert.trainable_variables + self.asr_bert.trainable_variables
